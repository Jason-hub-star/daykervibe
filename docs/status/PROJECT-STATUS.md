...
| Phase 1B: 협업 + 팀 페이지 | **완료** | `/camp`, `/rankings`, `/war-room/:teamId` | 원정대 모집, 글로벌 랭킹, 제출 준비 허브 반영 |
...
- `src/app/camp/page.tsx` — 원정대 모집 필터, 생성 폼, 모집 상태 카드
...
3. 애니메이션과 페이지 전환을 추가하되 기본 흐름 가독성을 해치지 않도록 조정한다.
