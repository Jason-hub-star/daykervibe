...
| 순서 | 작업 | 난이도 | 영향도 | 상태 |
|---|---|---|---|---|
| 1 | 공용 Modal 컴포넌트 | 낮음 | 높음 | **완료** — `src/components/ui/Modal.tsx` |
| 2 | 원정대 삭제 기능 | 중간 | 높음 | **SSOT 제외** — Wireframe이 "수정, 모집 마감"만 명시 |
| 3 | Ended 해커톤 생성 제한 | 낮음 | 중간 | **완료** — TeamsSection + SubmitSection 가드 검증 |
| 4 | 모집 역할 프리셋 칩 | 중간 | 중간 | **완료** — War Room TEAM MEMBERS roleLabel 칩 |
| 5 | 랭킹 닉네임 모달 | 중간 | 중간 | **SSOT 제외** — Schema에 모달용 추가 데이터 없음 |
| 6 | TeamCard 통합 | 중간 | 구조 개선 | **완료** — `src/components/ui/TeamCard.tsx` |
| 7 | 포맷 유틸 중앙화 | 낮음 | 구조 개선 | **완료** — `src/lib/format.ts` |

### 실행 결과 요약 (2026-03-10)
- SSOT 준수 5개 항목 구현 완료 (커밋 `e70830f`, `a4fa197`)
- SSOT 위반 2개 항목 의도적 제외 (팀 삭제, 랭킹 모달)
- 총 12개 파일 변경, +335줄 / -218줄
